...
class DWT_Function(Function):
    ...
    def forward(ctx, x, w_ll, w_lh, w_hl, w_hh):
        ...
        dim = x.shape[1]
        x_ll = torch.nn.functional.conv2d(x, w_ll.expand(dim, -1, -1, -1).float(), stride = 2, groups = dim)
        x_lh = torch.nn.functional.conv2d(x, w_lh.expand(dim, -1, -1, -1).float(), stride = 2, groups = dim)
        x_hl = torch.nn.functional.conv2d(x, w_hl.expand(dim, -1, -1, -1).float(), stride = 2, groups = dim)
        x_hh = torch.nn.functional.conv2d(x, w_hh.expand(dim, -1, -1, -1).float(), stride = 2, groups = dim)
    ...
    def backward(ctx, dx):
        if ctx.needs_input_grad[0]:
            ...
            dx = torch.nn.functional.conv_transpose2d(dx, filters.float(), stride=2, groups=C)
...
class IDWT_Function(Function):
    ...
    def forward(ctx, x, filters):
        ...
        x = torch.nn.functional.conv_transpose2d(x, filters.float(), stride=2, groups=C)
        return x
    ...
    def backward(ctx, dx):
        if ctx.needs_input_grad[0]:
            ...
            x_ll = torch.nn.functional.conv2d(dx, w_ll.unsqueeze(1).expand(C, -1, -1, -1).float(), stride = 2, groups = C)
            x_lh = torch.nn.functional.conv2d(dx, w_lh.unsqueeze(1).expand(C, -1, -1, -1).float(), stride = 2, groups = C)
            x_hl = torch.nn.functional.conv2d(dx, w_hl.unsqueeze(1).expand(C, -1, -1, -1).float(), stride = 2, groups = C)
            x_hh = torch.nn.functional.conv2d(dx, w_hh.unsqueeze(1).expand(C, -1, -1, -1).float(), stride = 2, groups = C)
