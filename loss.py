...
class Sobelxy(nn.Module):
    ...
    def forward(self,x):
        ...
        #return torch.sqrt(torch.pow(sobelx, 2) + torch.pow(sobely, 2) + 1e-6)
...
class Fusionloss(nn.Module):
    ...
    def forward(self,image_vis,image_ir,generate_img,weight0, weight1):
        image_y=image_vis[:,:1,:,:]
        ...
        #loss_in = wb0 * F.mse_loss(image_y, generate_img) + wb1 * F.mse_loss(image_ir, generate_img)
        y_grad=self.sobelconv(image_y)
        ir_grad=self.sobelconv(image_ir)
        generate_img_grad=self.sobelconv(generate_img)
        x_grad_joint=torch.max(y_grad,ir_grad)
        loss_grad=F.l1_loss(x_grad_joint,generate_img_grad)
        loss_total=(1* ssim_value)+(10*loss_in)+(1*loss_grad)
